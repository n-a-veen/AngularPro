...
export class LoginComponent {
  ...
  ngOnInit(): void {
    this.loginForm = this.formBuilder.group({
      username: ['algoSky@admin.com', [Validators.required, Validators.email]],
      password: ['algoSkyadmin', Validators.required],
    });
  ...
  // firebase
  email = 'algoSky@admin.com';
  password = 'algoSkyadmin';
  ...
  login() {
    ...
    if (this.validateForm(this.email, this.password)) {
      this.authservice
        ...
        this.toastr.success('log in successful','AlgoSky', {
          timeOut: 3000,
  ...
  Submit() {
    console.log(this.loginForm)
    if (
      this.loginForm.controls['username'].value === 'algoSky@admin.com' &&
      this.loginForm.controls['password'].value === 'algoSkyadmin'
    ) {
      this.router.navigate(['/home/dashboard']);
    } else {
